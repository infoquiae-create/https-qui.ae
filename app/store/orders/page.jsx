...
export default function StoreOrders() {
    ...
    const updateOrderStatus = async (orderId, status) => {
        try {
            const token = await getToken()
            await axios.put(`/api/store/orders/${orderId}`, {status}, {headers: { Authorization: `Bearer ${token}` }})
            setOrders(prev =>
                prev.map(order => 
    ...
    const updateTrackingDetails = async () => {
        if (!trackingData.trackingId || !trackingData.courier) {
            toast.error('Please fill in tracking ID and courier')
            return
        }

        try {
            const token = await getToken()
            await axios.put(`/api/store/orders/${selectedOrder.id}`, trackingData, {
            ...
            setSelectedOrder(prev => ({...prev, ...trackingData}))
            
            toast.success('Tracking details updated and customer notified!')
            setTrackingData({ trackingId: '', trackingUrl: '', courier: '' })
        } catch (error) {
            toast.error(error?.response?.data?.error || error.message)
        }
    }
